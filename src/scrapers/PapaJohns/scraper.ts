...
import { addVariant } from '../add-item'
 ...
 * near-identical rows — the same treatment Pizza Hut's sizes get.
 *
    ...
    async scrape (): Promise<RestaurantData> {
        ...
        for (const item of source.items ?? []) {
            ...
            for (const v of item.variants ?? []) {
                ...
                const outcome = addVariant(items, item.name, 'Size & Crust', v.label, built)
        ...
        console.log(
            chalk.green(
                `${this.icon} Papa Johns: ${variants} variants across ${source.items?.length ?? 0} products ` +
                `(extract ${source.source?.version ?? 'unknown'})`
            )
        )
        if (duplicates || implausible || rejected) {
            console.log(
                chalk.gray(
                    `   ↳ ${duplicates} duplicates, ${implausible} implausible, ` +
                    `${rejected} rows rejected during extraction (see nutrition.json)`
                )
            )
        }
